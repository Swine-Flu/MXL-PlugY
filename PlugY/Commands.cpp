...
const char* CMD_SET_PLAYERS = "/players";
const char* CMD_REFRESH_GAMBLE = "/refresh";
...
static Unit* GetUnitByUID(Game* ptGame, DWORD nUnitUID, DWORD nUnitType)
{
	for (Unit* ptUnit = *reinterpret_cast<Unit**>(reinterpret_cast<DWORD>(ptGame) + (nUnitType * 0x200) + (nUnitUID & 0x7F) * 4 + 0x1120); ptUnit != NULL; ptUnit = *reinterpret_cast<Unit**>(reinterpret_cast<DWORD>(ptUnit) + 0xE4)) {
		if (*reinterpret_cast<DWORD*>(reinterpret_cast<DWORD>(ptUnit) + 0x0C) == nUnitUID) {
			return ptUnit;
		}
	}
	return NULL;
...
static void* GetInteractionNode(Unit* ptNpc, Unit* ptUnit)
{
	void* ptInteractInfo = *reinterpret_cast<void**>(*reinterpret_cast<DWORD*>(reinterpret_cast<DWORD>(ptNpc) + 0x14) + 0x30);
	for (void* ptNode = *reinterpret_cast<void**>(ptInteractInfo); ptNode != NULL; ptNode = *reinterpret_cast<void**>(reinterpret_cast<DWORD>(ptNode) + 0x08)) {
		if (*reinterpret_cast<Unit**>(ptNode) == ptUnit) {
			return ptNode;
		}
	}
	return NULL;
...
static void* GetNpcRecord(Game* ptGame, DWORD nNpcUID)
{
	void* ptNpcControl = *reinterpret_cast<void**>(reinterpret_cast<DWORD>(ptGame) + 0x1D24);
	for (void* ptRecord = *reinterpret_cast<void**>(reinterpret_cast<DWORD>(ptNpcControl) + 0x04), *ptRecordEnd = reinterpret_cast<void*>(reinterpret_cast<DWORD>(ptRecord) + *reinterpret_cast<DWORD*>(ptNpcControl) * 0x44); ptRecord < ptRecordEnd; ptRecord = reinterpret_cast<void*>(reinterpret_cast<DWORD>(ptRecord) + 0x44)) {
		if (*reinterpret_cast<DWORD*>(ptRecord) == nNpcUID) {
			return ptRecord;
		}
	}
	return NULL;
...
static void* GetUnitProxy(void* ptNpcRecord, DWORD nPlayeUID)
{
	for (void* ptProxy = *reinterpret_cast<void**>(reinterpret_cast<DWORD>(ptNpcRecord) + 0x18); ptProxy != NULL; ptProxy = *reinterpret_cast<void**>(reinterpret_cast<DWORD>(ptProxy) + 0x08)) {
		if (*reinterpret_cast<DWORD*>(ptProxy) == nPlayeUID) {
			return ptProxy;
		}
	}
	return NULL;
...
void RefreshGamble(Unit* ptChar)
{
	...
	DWORD nInteractUID = *reinterpret_cast<DWORD*>(reinterpret_cast<DWORD>(ptChar) + 0x64);
	DWORD nInteractType = *reinterpret_cast<DWORD*>(reinterpret_cast<DWORD>(ptChar) + 0x68);

	if (!bInteraction || nInteractType != UNIT_MONSTER)
		return;

	Unit* ptVendor = GetUnitByUID(PCGame, nInteractUID, nInteractType);
	if (!ptVendor)
		return;

	if (ptVendor->nTxtFileNo != 147 && ptVendor->nTxtFileNo != 199 && ptVendor->nTxtFileNo != 254 && ptVendor->nTxtFileNo != 405 && ptVendor->nTxtFileNo != 512 && ptVendor->nTxtFileNo != 514)
		return;

	void* ptNode = GetInteractionNode(ptVendor, ptChar);
	if (!ptNode || *reinterpret_cast<DWORD*>(reinterpret_cast<DWORD>(ptNode) + 0x04) != 2)
		return;

	void* ptNpcRecord = GetNpcRecord(PCGame, ptVendor->nTxtFileNo);
	if (!ptNpcRecord)
		return;

	void* ptUnitProxy = GetUnitProxy(ptNpcRecord, *reinterpret_cast<DWORD*>(reinterpret_cast<DWORD>(ptChar) + 0x0C));
	if (!ptUnitProxy || *reinterpret_cast<BYTE*>(reinterpret_cast<DWORD>(ptUnitProxy) + 0x04) != 1)
		return;

	void* ptInteractInfo = *reinterpret_cast<void**>(*reinterpret_cast<DWORD*>(reinterpret_cast<DWORD>(ptVendor) + 0x14) + 0x30);
	if (*reinterpret_cast<void**>(ptInteractInfo) == ptNode && *reinterpret_cast<void**>(reinterpret_cast<DWORD>(ptNode) + 0x08) == NULL)
		D2FreeVendorStock(PCGame, ptChar, ptVendor);

	D2InitVendorStock(PCGame, ptChar, ptVendor, FALSE, TRUE);
}
...
int STDCALL commands (char* ptText)
{
	...
	if (!strncmp(command, CMD_SET_PLAYERS, strlen(CMD_SET_PLAYERS)))
	{
		if ((*ptGameMode != 0) && (*ptGameMode != 1) && (*ptGameMode != 6) && (*ptGameMode != 8))
			return 1;

		int nbPlayers = atoi(&command[strlen(CMD_SET_PLAYERS)]);
		nbPlayers = min(max(nbPlayers, 1), 8);

		D2SetNbPlayers(nbPlayers);
		_snprintf(ptText, MAX_CMD_SIZE, "players set to %d", nbPlayers);
		return 1;
	}
	...
	if (!strcmp(command, CMD_REFRESH_GAMBLE))
	{
		...
		if ((*ptPlayerInteraction != 1) || (*ptVendorInteraction != 3) || !(*ptActiveGambleMenu))
			return 0;

		DWORD nTickCount = GetTickCount();
		if ((nTickCount - *ptTransactionTimeStamp) < 500)
			return 0;

		*ptTransactionTimeStamp = nTickCount;

		D2ClearShopMenu(ptVendor);
...
void Install_Commands()
{
	...
	// Run custom commmand
	mem_seek R7(D2Client, 000000, 000000, 000000, 000000, 000000, 000000, 070AE6, 000000);
	memt_byte(0x90, 0xE8);
	MEMT_REF4(0x90909090, caller_Commands_mxls);
	memt_dword(0x90909090, 0x840FC085);
	memt_dword(0x90909090, 0x00000087);
	/*
