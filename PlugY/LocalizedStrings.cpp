...
void loadLocalizedStrings(int language)
{
	...
	{
		log_msg("Failed to load localized strings from mpq.\n");
		if (!iniFile->InitReadWrite("PlugY\\LocalizedStrings.ini", INIFILE_READ, 0))
		{
			log_msg("Failed to load localized strings from file.\n");
			exit(-1);
		}
	}
