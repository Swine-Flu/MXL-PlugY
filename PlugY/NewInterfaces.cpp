...
#include "statsPoints.h"	// Install_StatsPoints()
...
{
	if (onRealm) { D2DrawStatsPage(); return; }

	if (active_StatsPoints && selectedPage == 0)
		printNewStatsPage();
...
FCT_ASM ( caller_mouseCustomPageLeftDown_mxls )
...
FCT_ASM ( caller_mouseCustomPageLeftUp_mxls )
	MOV ESI, DWORD PTR SS:[ESP + 0x1C]
	PUSH ESI
...
FCT_ASM ( caller_resetSelectedPage_113 )
	MOV selectedPage, 0
	RETN
}}
...
void Install_NewInterfaces()
{
	...
	log_msg("Patch D2Client & D2Sigma for new stats/custom page interface. (InterfaceStats/NewInterfaces)\n");
	...
	if (selectMainPageOnOpenning)
	{
		...
		mem_seek R7(D2Client, 000000, 000000, 000000, 000000, 000000, 000000, 065E8B, 000000);
		MEMT_REF4(0xFFFA60FD, caller_resetSelectedPage_113);
		/*
