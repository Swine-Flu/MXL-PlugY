...
void Install_SavePathConversionFix()
{
	static int isInstalled = false;
	if (isInstalled) return;
	...
	log_msg("Patch D2Sigma to fix save path string conversion. (SavePathConversionFix)\n");

	mem_seek(offset_D2Sigma + 0x60215D);
	MEMT_REF4(0x0001BD91, caller_wcstombs_mxls);
	memt_byte(0x83, 0x90);
	memt_byte(0xC4, 0x90);
	memt_byte(0x0C, 0x90);
