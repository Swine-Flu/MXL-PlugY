...
int FASTCALL handleServerUpdate(Game* ptGame, Unit* ptChar, BYTE* ptPacket, DWORD nSize)
{
	d2_assert(!ptPacket || ptPacket[0] != 0x4A || nSize != 3, "Server update packet has invalid signature", __FILE__, __LINE__);

	int type = ptPacket[1];
	DWORD arg = ptPacket[2];
	...
	switch(type)
	{
		...
		case US_REFRESH_GAMBLE:
			RefreshGamble(ptChar);
			return 1;
...
void Install_UpdateServer()
{
	...
	mem_seek R7(D2Net, 000000, 000000, 000000, 000000, 000000, 000000, 00AD00, 000000);
	memt_dword(0x00000000, 0x00000003);
